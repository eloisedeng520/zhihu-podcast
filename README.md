...
当前 `app/page.tsx` 中的 `SKIP_ZHIHU_LOGIN = false`，已恢复知乎登录要求。临时测试时改为 `true` 会跳过知乎登录页面，可直接发现内容、制作播客、收藏和使用「我的制作」。测试记录沿用当前浏览器的匿名身份；更换浏览器或清除浏览器存储后不会沿用该身份。此开关不替代文本模型、语音等服务的配置。

恢复登录要求时，将该开关改为 `false`。正式部署前关闭此临时开关；已构建的版本需要重新构建才能生效。
