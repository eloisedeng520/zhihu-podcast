...
import type {Answer,ImageAsset,KnowledgeItem,KnowledgeCategory} from "../lib/podcast.ts";
...
const servedImages=(images?:ImageAsset[])=>images?.map(image=>({...image,url:image.originalUrl||image.url}));
...
export function localKnowledgeItems():KnowledgeItem[]{
  const singles=records.filter(record=>record.category!=="hot"||!record.collectionId).map(({content,fetchedAt,url,...item})=>({...item,images:servedImages(item.images),labels:[item.sourceName,item.author,item.category]}));
...
export function localAnswer(id:string):Answer{
  if(id.startsWith(hotQuestionPrefix)){
    ...
    for(const [answerIndex,record] of answers.entries()){
      ...
      paragraphs.push(...answer.paragraphs.map((paragraph,index)=>({id:`a${answerIndex+1}p${index+1}`,text:paragraph.text,images:index===0?servedImages(record.images):undefined})));
    }
    return {id,title:answers[0].title,author:`${contributors.length} 位答主`,url:`https://www.zhihu.com/question/${questionId}`,paragraphs,images:answers.flatMap(answer=>servedImages(answer.images)||[]),contributors,fetchedAt:answers[0].fetchedAt};
  }
  ...
  if(record.images?.length){ const images=servedImages(record.images); answer.images=images; if(answer.paragraphs[0])answer.paragraphs[0].images=images; }
